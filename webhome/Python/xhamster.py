    ...
    def getlist(self, data):
        ...
        for i in data.items():
            vlist.append({
                ...
                'vod_name': i('.video-thumb-info a').text().strip() or i('a[aria-label]').attr('aria-label') or i('img[alt]').attr('alt') or '',
                'vod_pic': i('.role-pop img').attr('src') or '',
                'vod_remarks': i('.role-pop div[data-role="video-duration"]').text().strip() or '',
                'style': {'ratio': 1.33, 'type': 'rect'}
            })
        return vlist
