...
class Spider(Spider):
    def init(self, extend=""):
        ...
        try:
            ext = _json.loads(extend) if isinstance(extend, str) and extend.startswith('{') else (extend if isinstance(extend, dict) else {})
        except: ext = {}
        proxy = str(ext.get('proxy') or '').strip()
        if proxy:
    ...
    def _ensure_proxy(self):
        if self._proxy_checked:
            return
        self._proxy_checked = True
        for port in (18890, 7890, 2080, 10809):
            p = 'http://127.0.0.1:%d' % port
            try:
                self.session.proxies = {'http': p, 'https': p}
                self.session.get(self.host, timeout=4, verify=False)
                print('[Beeg proxy] 使用 %s' % p)
                return
            except Exception:
                continue
        self.session.proxies = {}
        print('[Beeg proxy] 未找到可用代理，直连')
    ...
    def fetch(self, url, params=None, headers=None, timeout=None):
        ...
        for _ in range(self.retries + 1):
            try:
                resp = self.session.get(url, params=params, headers=headers, timeout=timeout or self.timeout, verify=False)
                resp.encoding = 'utf-8'
                return resp
