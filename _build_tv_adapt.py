...
(function () {
  ...
  /* ---------- 拦截 popstate：弹层打开时遥控器返回键不应关闭播放页 ----------
     ...
     导致音质/列表弹框开着时按返回，播放页被原应用直接关闭。
     双重保险：
     1) popstate 捕获阶段拦截 + 恢复 #player 历史
     2) MutationObserver 兜底：弹层开着时若 #np.open 被移除，立即加回 */
  ...
  window.addEventListener("popstate", function (e) {
    ...
    if (hasOverlay || (now - _overlayJustClosed) < 600) {
      ...
      try { history.pushState({ aqNp: 1 }, "", "#player"); } catch (eh) {}
      showDiag(["popstate blocked (overlay open/recently closed)"]);
    }
  }, true);
  ...
  function doBack(L) {
    ...
    if (L.kind === "editor") { _overlayJustClosed = Date.now(); click(document.querySelector(".src-editor-mask.open")); return; }
    if (L.kind === "qpop") { _overlayJustClosed = Date.now(); click($("qPopMask")); return; }
    if (L.kind === "sheet") { _overlayJustClosed = Date.now(); click($("sheetMask")); return; }
    if (L.kind === "plat") { _overlayJustClosed = Date.now(); click($("btnPlat")); return; }
