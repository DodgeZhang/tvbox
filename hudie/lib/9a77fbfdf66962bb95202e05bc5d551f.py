...
class Spider(BaseSpider):
    CONFIG = {
        ...
        # [修改] 原本是 juziapp.hzhcbkj.cn,服务端对这个域名做了安全码拦截(返回 40003),
        # 但通过 IP 直连 103.36.167.27:18004 是同一个后端,且 /api/ex/v3/security/tag/list 不再拦截,
        # 所以主页视频能正常拿到。init() 会先用这里的 host,site 配置不会覆盖(只 host 为空才覆盖)
